...
<div id="dashboard_container">
    <div class="Box" id="View_Upcome_Trips">
        <div class="Box_Head msgbg"><h2><?php echo translate("Your Trips"); ?><span class="View_MyPrint"> <a href="javascript:void(0);" onclick="javascript:print_reservation();"><?php echo translate("Print this page"); ?></a> </span></h2></div>
        ...
        <div class="Box_Content">
            ...
            <?php if ($result->num_rows() > 0)
                        ...
                        foreach ($result->result() as $row)
                        {
                            if ($this->db->where('id', $row->list_id)->get('list')->num_rows() != 0)
                            {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <p> <img height="50" width="50" alt="image" style="float:left; margin:0 10px 10px 0;" src="<?php echo $this->Gallery->profilepic($row->userto, 2); ?>" />
                                            <span class="clsBold">
                                                ...
                                                <a href="<?php echo site_url('users/profile') . '/' . $row->userto; ?>">
                                                    <?php
                                                    if (get_user_by_id($row->userto))
                                                    {
                                    ...
                                    <td  width="20%"><?php echo date("F j, Y", $row->checkin) . ' - ' . date("F j, Y", $row->checkout); ?></td>
                                    <td>
                                            <?php if (date('m/d/Y', get_user_time($row->checkin, get_user_timezone())) == date('m/d/Y', get_user_time(local_to_gmt(), get_user_timezone())))
                                            {
                                                ?>
                                            <p class="clsBold"><?php
                                            if ($row->status == 3)
                ...
                <?php } ?>
                                ...
                                <?php if ($row->status < 8 && $row->status != 5 && $row->status != 6 && $row->status != 4 && $row->status != 2)
                                {
                                    ?>
